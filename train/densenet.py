...
from tensorflow.keras.layers import Dense, Dropout, Activation, Permute
...
def dense_cnn(input, nclass):
    ...
    if g_nb_filter == 128:
        ...
        # 192
        x, _nb_filter = transition_block(x, 192, _dropout_rate, 2, _weight_decay)
        # 192 + 8 * 8 = 256
        x, _nb_filter = dense_block(x, 8, _nb_filter, 8, None, _weight_decay)
        # 256 -> 192
        x, _nb_filter = transition_block(x, 192, _dropout_rate, 2, _weight_decay)
...
input = Input(shape=(60, 200, 1), name='the_input')
